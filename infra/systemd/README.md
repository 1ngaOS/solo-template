...
|----------|-------------|----------------------------------|------------------------|
| `main`   | **staging** | `/opt/<APP_NAME>/backend/staging`, `/opt/<APP_NAME>/frontend/staging` | `backend-staging`, `frontend-staging` |
| `prod`   | **production** | `/opt/<APP_NAME>/backend/production`, `/opt/<APP_NAME>/frontend/production` | `backend-production`, `frontend-production` |
...
2. Install the example systemd units:
   ...
   ```
   Edit each unit so paths use your `APP_NAME` (e.g. `/opt/app/` → `/opt/myapp/`).
